...
impl Function {
    ...
    pub fn parse(expression: Expression) -> Result<Self, String> {
        ...
        if argument_names_tokens.iter().any(|token| {
            if let Token::Identity(_) | Token::ArgumentSeparator = token {
                false
            } else {
                true
            }
        ...
        for token in argument_names_tokens.into_iter() {
            match token {
                Token::Identity(identity) if expected_token_type.is_identity() => {
                    ...
                    expected_token_type = Token::ArgumentSeparator
                }
        ...
        if let &[Token::Operator(Operator::CloseParen), Token::FunctionAssignment] =
...
impl MathFunction for Function {
